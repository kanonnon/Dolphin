...
//とりあえずどんなものかを見るために調べたコードをコピペしてみた。RecordViewControllerで入力した日付をx軸、タイムをy軸に表したい。タイムはy軸の正方向にタイムが速く(数が小さく)数が小さくなるようにしたい。グラフのy軸の値はグラフにする種目によって大幅に異なってくるから、そのメモリを自動的に変えることは可能か？種目ごとにグラフを分けたいが自動で分けてグラフを複数個作ることは可能か？自動でできないならユーザーが設定した種目の分だけグラフにすることは可能か？
